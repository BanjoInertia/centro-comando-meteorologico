      ...
      <div className="bg-slate-950/85 border border-slate-800 rounded-xl shadow-2xl backdrop-blur-md overflow-hidden">
        ...
        >
          <div className="flex gap-1.5 items-center">
            {CATEGORIES.map((c) => (
              <span key={c.label} className={`inline-flex w-3 h-3 rounded-full ${c.dotClass}`} />
            ))}
          </div>
          <span className="font-mono text-[13px] font-bold tracking-widest text-slate-300 uppercase">
            Categoria de Voo
          </span>
          {isExpanded
          ...
          <div className="border-t border-slate-800 px-3 pb-3.5 pt-2.5 flex flex-col gap-2.5 min-w-[240px]">
            {CATEGORIES.map((c) => (
              <div key={c.label} className="flex items-start gap-3">
                <span className={`mt-0.5 inline-flex w-3.5 h-3.5 rounded-full flex-shrink-0 ${c.dotClass}`} />
                <div className="flex flex-col">
                  <div className="flex items-baseline gap-2">
                    <span className={`font-mono text-[13px] font-extrabold ${c.textClass}`}>{c.label}</span>
                    <span className="text-xs text-slate-300 font-medium">{c.desc}</span>
                  </div>
                  <span className="text-[11px] text-slate-500 font-mono leading-tight">{c.sub}</span>
                </div>
              </div>
            ))}
          </div>
